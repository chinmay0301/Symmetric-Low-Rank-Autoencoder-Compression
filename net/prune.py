    ...
    def prune_vae_by_std(self, s=0.25):
        for name, module in self.named_modules():
            if name in ['encoder_l1', 'encoder_l2', 'encoder_l3', 'decoder_l1', 'decoder_l2', 'decoder_l3']:
                ...
                print(f'Pruning with threshold : {threshold} for layer {name}')
                module.prune(threshold)
    def prune_vae_by_std_symm(self, s=0.25):
        threshold= {}
        for name, module in self.named_modules():
            if name in ['encoder_l1', 'encoder_l2', 'encoder_l3', 'decoder_l1', 'decoder_l2', 'decoder_l3']:
                threshold[name] = np.std(module.weight.data.cpu().numpy()) * s
                if name in ['decoder_l1', 'decoder_l2', 'decoder_l3']:
                    threshold[name] = threshold['encoder_l' + name[-1]]
                    print(f'Pruning with threshold : {threshold[name]} for layer {name}')
                module.prune(threshold[name])
